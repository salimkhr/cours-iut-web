...
export default function PageFooter({}: PageFooterProps = {}) {
    return (
        <footer
            className="w-full bg-brand-dark text-brand-gray-300 mt-auto opacity-0 animate-fade-in z-10">
            <div className="mx-auto max-w-6xl px-6 lg:px-12 py-12 lg:py-16 grid grid-cols-1 md:grid-cols-3 gap-10">
                <div className="flex flex-col gap-4">
                    <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-lg font-bold text-white">Cours Web</h3>
                    </div>
                    <p className="text-sm leading-relaxed text-brand-gray-500 max-w-xs">
                    ...
                    <ul className="flex flex-col gap-2">
                        {navItems.map((item) => (
                            <li key={item.href}>
                                <Link
                                    href={item.href}
                                    className="text-sm text-brand-gray-500 hover:text-brand-accent transition-colors"
                                >
                                    {item.label}
                                </Link>
                            </li>
                        ...
                        {socialItems.map(({label, href, icon: Icon}) => (
                            <li key={href}>
                                <Link
                                    ...
                                    className="inline-flex items-center gap-2 text-sm text-brand-gray-500 hover:text-brand-accent transition-colors"
                                >
                                    <Icon size={16}/>
                                    {label}
                                </Link>
            ...
            <div className="border-t border-brand-gray-700/40">
                <div
                    className="mx-auto max-w-6xl px-6 lg:px-12 py-6 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-brand-gray-500">
