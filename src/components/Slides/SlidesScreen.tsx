...
export const SlidesScreen: React.FC<SlidesScreenProps> = ({children, module, section}) => {
    ...
    if (!mounted) {
        return (
            <div
                className="relative flex flex-col items-center justify-center min-h-[600px] w-full bg-muted/30 rounded-2xl p-4 border animate-pulse">
                ...
                {isMobile ? (
                    ...
                    <>
                        ...
                        {showNotes && currentNotes && (
                            <div
                                className="absolute top-4 left-4 z-40 w-80 max-h-[70vh] overflow-y-auto bg-background/95 backdrop-blur-md p-6 rounded-2xl border border-border shadow-2xl animate-in fade-in slide-in-from-left-4 duration-300">
                                <h3 className="text-sm font-semibold mb-4 text-primary uppercase tracking-wider">Notes
                                    de présentation</h3>
