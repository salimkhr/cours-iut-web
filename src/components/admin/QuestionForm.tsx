...
export default function QuestionForm({ open, onOpenChange, mode, question, onSubmit }: QuestionFormProps) {
  ...
  useEffect(() => {
    if (mode === "edit" && question) {
      reset({
        text: question.text,
        type: question.type,
        choices: question.type === "multiple-choice" ? (question.choices || []) : ["", ""],
      ...
      reset({ text: "", type: "multiple-choice", choices: ["", ""], correctAnswer: "", points: 1, timeLimit: undefined, explanation: "", difficulty: undefined });
    }
  }, [mode, question, reset]);
  ...
  useEffect(() => {
    ...
    if (!watch("choices") || (watch("choices") as unknown as string[])?.length < 2) {
      setValue("choices", ["", ""] as unknown as string[]);
    }
      ...
      <DialogContent>
        <form onSubmit={handleSubmit((data) => onSubmit(data))} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <Label>Points</Label>
              <Input type="number" step="1" min="0" {...register('points', { required: 'Points requis', valueAsNumber: true })} />
            ...
            <div>
              ...
              <Select onValueChange={(v: Difficulty) => setValue('difficulty', v)} defaultValue={question?.difficulty}>
                <SelectTrigger className="w-full">
                ...
                <div className="space-y-2">
                  {fields.map((field, index) => (
                    <div key={field.id} className="flex gap-2">
                      <Input className="flex-1" {...register(`choices.${index}` as const, { required: true })} />
                      <Button type="button" variant="outline" onClick={() => remove(index)} disabled={fields.length <= 2}>Supprimer</Button>
                    </div>
                  ))}
                  <Button type="button" variant="outline" onClick={() => append("")}>Ajouter un choix</Button>
                </div>
                <div>
                  <Label>Réponse correcte</Label>
                  <Select onValueChange={(v: string) => setValue('correctAnswer', v)} defaultValue={question?.correctAnswer}>
                    <SelectTrigger className="w-full">
