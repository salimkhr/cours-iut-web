...
export default function AdminQuestions() {
  ...
  useEffect(() => { fetchQuestions(); }, [params.q, params.type, params.difficulty]);
  ...
  const openAdd = () => { setMode('add'); setEditingQuestion(undefined); setOpenForm(true); };
  const openEdit = (q: Question) => { setMode('edit'); setEditingQuestion(q); setOpenForm(true); };
            ...
            {questions.map(qq => (
              <TableRow key={String(qq._id)}>
                ...
                <TableCell className="text-right space-x-2">
                  <Button size="sm" variant="outline" onClick={() => openEdit(qq)}><Pencil className="size-4"/></Button>
                  <Button size="sm" variant="destructive" onClick={() => onDelete(qq)}><Trash2 className="size-4"/></Button>
                </TableCell>
              </TableRow>
            ))}
            {questions.length === 0 && (
              <TableRow>
                <TableCell colSpan={6} className="text-center text-muted-foreground">{loading ? 'Chargement...' : 'Aucune question'}</TableCell>
              </TableRow>
            )}
          </TableBody>
        </Table>
