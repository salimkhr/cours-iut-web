    ...
    return (
        <article>
            <section>
                ...
                <Text>
                    Lorsqu&apos;un événement est déclenché sur un élément du DOM, il traverse plusieurs
                    phases avant d&apos;être complètement traité. Ces phases définissent le parcours de
                    l&apos;événement à travers l&apos;arbre DOM :
                </Text>
                ...
                <List>
                    <ListItem>
                        <strong>Phase de capture</strong> : l&apos;événement commence son parcours à partir
                        de l&apos;élément racine du document (<Code>document</Code>) et descend dans
                        l&apos;arbre DOM jusqu&apos;à l&apos;élément cible. Pendant cette phase, les
                        gestionnaires enregistrés avec le paramètre <Code>capture: true</Code> sont
                        exécutés.
                    </ListItem>
                    <ListItem>
                        <strong>Phase de cible</strong> : l&apos;événement atteint l&apos;élément sur
                        lequel il a été déclenché (l&apos;élément cible). Les gestionnaires
                        d&apos;événements enregistrés directement sur cet élément sont exécutés.
                    </ListItem>
                    <ListItem>
                        <strong>Phase de bouillonnement</strong> : une fois que l&apos;événement a atteint
                        l&apos;élément cible, il commence à &laquo;&nbsp;bouillonner&nbsp;&raquo; en
                        remontant à travers l&apos;arbre DOM, en passant par ses ancêtres directs jusqu&apos;à
                        l&apos;élément racine. Pendant cette phase, les gestionnaires enregistrés sans le
                        paramètre <Code>capture: true</Code> sont exécutés.
                    </ListItem>
                </List>
                ...
                <Text>
                    Par défaut, les gestionnaires d&apos;événements sont déclenchés pendant la phase de
                    bouillonnement. Cela signifie que, si un gestionnaire est ajouté à un parent d&apos;un
                    élément cible, ce gestionnaire sera exécuté lorsque l&apos;événement remontera de
                    l&apos;élément cible à travers ses ancêtres.
                </Text>
...
// - "Clic sur le parent (bouillonnement)" (phase de bouillonnement)`}
                ...
                <Text>
                    Par défaut, les gestionnaires d&apos;événements s&apos;exécutent dans la phase de
                    bouillonnement. Toutefois, il est possible d&apos;intercepter un événement pendant la
                    phase de capture en passant <Code>{`{ capture: true }`}</Code> ou <Code>true</Code>{" "}
                    comme troisième argument à <Code>addEventListener</Code>.
                </Text>

                <CodeCard language="javascript">
                    {`// Ajout de gestionnaires pour la phase de capture
...
// - "Clic sur le parent (bouillonnement)" (phase de bouillonnement)`}
                ...
                <Text>
                    Si vous souhaitez empêcher un événement de continuer à se propager, utilisez la méthode{" "}
                    <Code>event.stopPropagation()</Code>. Cela arrête l&apos;événement au niveau actuel et
                    empêche son passage aux phases suivantes (capture ou bouillonnement).
                </Text>
...
// Résultat dans la console :
// - "Clic sur l'enfant" (phase de cible)
// - Le clic sur le parent n'est pas enregistré : la propagation est arrêtée.`}
                </CodeCard>

                <Heading level={3}>4. Pourquoi utiliser la phase de bouillonnement ?</Heading>
                <Text>
                    La phase de bouillonnement est souvent utilisée pour une fonctionnalité appelée
                    <strong> délégation d&apos;événements</strong>. Elle permet d&apos;attacher un
                    gestionnaire d&apos;événements à un parent pour gérer les événements de ses enfants,
                    même si ces derniers sont ajoutés dynamiquement. Cela réduit le nombre de gestionnaires
                    nécessaires et améliore les performances.
                </Text>
            </section>

            <section>
                <Heading level={2}>B- La délégation d&apos;événements</Heading>
                <Text>
                    La délégation consiste à attacher un écouteur d&apos;événement à un parent commun pour
                    gérer plusieurs enfants, souvent créés dynamiquement. Cela améliore les performances et
                    simplifie le code.
                </Text>
...
data.forEach((item) => {
    ...
    list.appendChild(li);
                ...
                <List>
                    <ListItem>
                        Utilisez la propriété <Code>event.target</Code> pour identifier l&apos;élément
                        cible.
                    </ListItem>
                    <ListItem>La délégation réduit le nombre d&apos;écouteurs attachés.</ListItem>
                    <ListItem>
                        Elle fonctionne automatiquement pour les éléments ajoutés dynamiquement après
                ...
                <List>
                    <ListItem>
                        <strong><Code>parentElement</Code></strong> : permet d&apos;accéder à
                        l&apos;élément parent direct.
                    </ListItem>
                    <ListItem>
                        <strong><Code>children</Code></strong> : permet d&apos;accéder à tous les éléments
                        enfants directs.
                    </ListItem>
                    <ListItem>
                        <strong><Code>closest(selector)</Code></strong> : permet de trouver l&apos;ancêtre
                        le plus proche correspondant à un sélecteur CSS. La méthode remonte dans
                        l&apos;arbre DOM (en commençant par l&apos;élément lui-même) pour trouver le
                        premier ancêtre correspondant. Si aucun ancêtre ne correspond, elle retourne{" "}
                        <Code>null</Code>.
                    </ListItem>
                </List>

                <DiagramCard
                    chart={closestDiagram}
                    header="Navigation dans le DOM avec closest() et parentElement"
                />
...
const container = child.closest(".container");`}
                ...
                <Text>
                    La méthode <Code>preventDefault()</Code> empêche le comportement par défaut d&apos;un
                    événement (comme la soumission d&apos;un formulaire ou le suivi d&apos;un lien). Elle
                    n&apos;arrête pas la propagation de l&apos;événement, contrairement à{" "}
                    <Code>stopPropagation()</Code>.
                </Text>

                <CodeCard language="javascript">
                    {`// Exemple avec preventDefault sur un lien
